...
def create_supplier(
    ...
    try:
        result = create_supplier_record(session, payload, current_user)
    except ServiceError as error:
        _raise_service_error(error)

    log_action(session, actor_user=current_user, action="supplier_created", entity_type="supplier", entity_id=result.id, request=request, metadata={"name": result.name})
    session.commit()
    return result
...
def patch_supplier(
    ...
    try:
        result = update_supplier_record(session, supplier_id, payload, current_user)
    except ServiceError as error:
        _raise_service_error(error)

    log_action(session, actor_user=current_user, action="supplier_updated", entity_type="supplier", entity_id=supplier_id, request=request)
    session.commit()
    return result
